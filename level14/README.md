...
Target user is `flag14`. Let's see what we have:

```sh
ls -lA
```

Nothing, like good old-fashioned level00. Let's see system-wide:

```sh
find / -user flag14 2>/dev/null # nothing
find / -user level14 2>/dev/null # nothing useful
find / -readable 2>/dev/null # same
```

That sucks.

```sh
ls -lA "$(getent passwd flag14 | cut -d ':' -f 6)"
```

Output:

```
ls: cannot open directory /home/flag/flag14: Permission denied
```

Son of a witch. There's only one thing left we haven't examined in any level yet: the `getflag` binary itself.
...
It's pretty much the same as `level13`, except it holds every token of every flag user.
UID of `flag14` is decimal 3014 (0xbc6). We can look up in the code the encrypted flag:
...
Output: `7QiHafiNa3HVozsaXkawuYrTstxbpABHD8CPnHJ`

Since there is no user `level15`, we should try this as the password for `flag14`:

```
flag14@localhost's password: 7QiHafiNa3HVozsaXkawuYrTstxbpABHD8CPnHJ
Congratulation. Type getflag to get the key and send it to me the owner of this livecd :)
```

It worked. Let's type `getflag`:
